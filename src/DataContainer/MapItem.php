...
class MapItem extends CoreContainer
{
    ...
    public function toggleIcon(
        ...
        return '<a href="' . $this->addToUrl($href) . '" title="' . StringUtil::specialchars(
            $title
    ...
    public function toggleVisibility(
        ...
        if (
            ...
            foreach ($GLOBALS['TL_DCA']['tl_wem_map_item']['fields']['published']['save_callback'] as $callback) {
                if (\is_array($callback)) {
                    ...
                    $blnVisible = $this->{$callback}[0]->{$callback}[1]($blnVisible, $dc instanceof DataContainer ? $dc : $this);
                } elseif (\is_callable($callback)) {
                    $blnVisible = $callback($blnVisible, $dc instanceof DataContainer ? $dc : $this);
                }
            }
        }
        ...
        $this->Database->prepare(
            'UPDATE tl_wem_map_item SET tstamp=' . time() . ", published='" . ($blnVisible ? '1' : '') . "' WHERE id=?"
        )
            ->execute($intId)
        ;
